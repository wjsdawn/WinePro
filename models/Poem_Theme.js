...
const query = "match(n:poem) -[r]- (m:theme) return n,m,r"
const getJson = async() =>{
        ...
        neo4jDriver.session().run(query)
            .then(result=>{
                ...
                result.records.forEach(record=>{
                    ...
                    if((JSON.stringify(node).indexOf(JSON.stringify({'name':record.get('m').properties.theme,type:'theme'})))==-1){
                        node.push({'name':record.get('m').properties.theme,type:'theme'})
                    }
                })
                //设置边
                result.records.forEach(record=>{
                    let source,target;
                    let option_source = {name:record.get('m').properties.theme}
